...
`tuicr pr` with no target, or with a local branch name, uses `LocalForgeBackend`. Local forge reads only the checkout's git objects and the existing structured Git diff adapter; it never shells out, calls a network forge CLI, or writes inside the checkout or `.git`. Its numbered pull, review, and thread records live under the tuicr data directory. `ForgeBackend::resolve_thread` is implemented only by Local; other backends use the default unsupported-operation error. The diff-watch tick follows a Local pull request's head branch and reuses the existing PR reload path when its tip moves.
