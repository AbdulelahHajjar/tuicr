...
local_pr_follow_interval_ms = 1000
...
| `local_pr_follow_interval_ms` | `1000`    | Poll interval for following the head branch of a Local pull request and for picking up review-thread changes written by other processes (an agent's `tuicr review reply`, for example) without `:e`. `0` disables both. Ignored for other forges. |
